...
## 🏆 提交说明（Hackathon Submission）

> 赛道：**🟩 交易 Infra**

### 第一段 · 思路

**痛点**：AI 交易智能体能力很强，但作为黑盒是危险的。一个能"感知市场 → 形成策略 → 下单 → 管风险"的自主 Agent 一旦出问题，用户根本看不清它"为什么"这么做——工具调用、回测、风控、审批、执行散落在各种日志 / 面板 / 聊天里，没有以"一次运行"为单位的标准审计轨迹，失败或危险的运行也没法清晰回放、做不了复盘。

**解法**：TradeTrace（交易智能体版的黑匣子 + 驾驶舱语音记录器）补的是「自主智能体」与「交易执行」之间缺失的那层治理基础设施，把每一次运行变成可回放、可审计的轨迹：

```text
自然语言策略 → Qwen 解析 → Bitget Skill 证据包 → 本地确定性回测（真实 Bitget 公共 K线）
            → 规则化风险账本 → 人在回路审批闸门 → 模拟 / replay 执行 → 运行后报告
```

三个关键设计决策：

1. **风险评分基于规则、确定性、可解释**——LLM 只负责解析策略与生成报告，绝不参与最终安全决策；
2. **回测基于 Bitget 官方公共行情、真实可复现**——直接打 `https://api.bitget.com/api/v2/spot/market/candles`（Agent Hub `spot_get_candles` / `futures_get_candles` 同源接口），不造假；
3. **默认执行层是 replay 适配器，明确不发送真实订单**，执行前始终有可审计的闸门。

### 第二段 · 完成度

**已完成**：完整的"记录 → 回测 → 风控 → 审批 → 执行 → 报告 → 回放"治理骨架；两个入口（Web UI + Telegram bot，共享同一份 Run 存储）；确定性风险规则引擎（命中即拦截马丁格尔 / 高杠杆 / 无止损等危险策略，决策可降级到 Go / Review / Block）；本地确定性回测引擎（EMA 交叉 / RSI / 动量，逐根 bar 模拟，真实 PnL / Sharpe / 回撤 / 胜率）；飞行记录器式时间线、人在回路审批、运行后报告、结构化日志落盘。所有核心能力封装在边界清晰的工具模块（[agent/tools/](agent/tools/)），每一层都有明确替换点，可作为脚手架二次开发。

**遇到的问题与解法**：

- 官方 Playbook 不可达 → 改为**本地确定性回测**，反而更可复现（评委可 `npm run reproduce` 一键核验）；
- 五个 Skill Hub 技能未暴露固定 REST 端点 → 用 Qwen **模拟五个 persona**，保证本地与部署行为一致，留好替换点；
- 回测失败时面临"静默降级"的诱惑 → 坚持**显式标记 `failed` 状态**并把真实原因显示在 UI 上，绝不掩盖。

**尚未实现 / 下一步**：真实下单适配器（默认 replay，明确不接真实订单）；WebSocket 私有行情与深度；Playbook / Agent Hub 编排接入；MCP server 封装。

**技术栈与 Bitget 工具**：Next.js（App Router, TypeScript）、Qwen 通义千问（策略解析 + 报告生成，走 Bitget 代理 `qwen3.6-plus`）、Zod。**`agent/` 目录遵循 [Vercel Eve](https://vercel.com/eve)「an agent is a directory」的文件约定**（`agent.ts` + `instructions.md` + `tools/ / skills/ / subagents/ / channels/ / schedules/`），便于后续接入 Eve runtime；当前运行编排由 Next.js + 自写的 `agent.ts` 生命周期驱动。**Bitget 工具**：直接调用 Bitget **公共行情 API**（Agent Hub `spot_get_candles` / `futures_get_candles` 同源，回测无需 key）；参考 **Skill Hub** 五个分析师 persona 设计证据包优先级。

**评委一键复现**：`npm run reproduce` 读取样本 fixture 里已解析的策略，喂给运行时同一个回测引擎，在真实 Bitget 公共 K线上重跑并和记录值对比（交易数 / 胜率 / 最大回撤逐位一致）。详见 [运行日志与复现](#运行日志与复现)。

### 第三段 · 对 AI Trading 的看法（选填）

Agentic Trading 真正的瓶颈不在"能不能自主下单"——模型能力已经够了——而在**信任与可问责**。一个你睡觉时自己跑的 Agent，如果没法回答"它当时为什么这么做"，就永远进不了真实资金。TradeTrace 的实践让我相信，这一代 AI 交易基础设施的核心不是更聪明的策略，而是把"为什么"显式化：可解释的风控、可回放的轨迹、人在回路的闸门。这是 LLM 从"demo"走向"production agent"必须补上的一层。

---

## 目录

- [🏆 提交说明（Hackathon Submission）](#-提交说明hackathon-submission)
- [它解决什么问题](#它解决什么问题)
- [安装](#安装)
- [快速开始](#快速开始)
- [接入方式](#接入方式)
...
回测结果不接受纯截图——附生成它的代码并允许复现核验。仓库内置一条复现命令：

```bash
npm run reproduce
```

它会读取样本 fixture [`samples/run-success.json`](samples/run-success.json) 里**已解析好的策略**，喂给运行时同一个回测引擎（[agent/tools/local-backtest.ts](agent/tools/local-backtest.ts) 的 `runLocalBacktest`），在**真实 Bitget 公共 K线**（BTCUSDT，1h）上重跑，然后把重算的指标和 fixture 里记录的值并排打印：
...
确定性部分（交易数、胜率、最大回撤）逐位一致；PnL/Sharpe 会随真实行情推进而漂移（fixture 在 2026-06-20 捕获，复现时拉的是最新 K线）——**这是真实数据方差，不是造假**。脚本源码：[scripts/reproduce-backtest.mts](scripts/reproduce-backtest.mts)，无需 Qwen key、无需 Bitget key。
...
├── agent/                     # 遵循 Vercel Eve「an agent is a directory」约定
